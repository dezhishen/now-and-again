...
<template>
  ...
  <button class="flex-1 text-xs py-1.5 rounded-lg bg-gray-50 dark:bg-gray-700/50 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors font-medium" @click="$emit('skip', todo)">⏭️ {{ t('todo.skip') }}</button>
  <button class="flex-1 text-xs py-1.5 rounded-lg bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-300 hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors font-medium" @click="$emit('interrupt', todo)">⛔ {{ t('todo.interrupt') }}</button>
</template>
