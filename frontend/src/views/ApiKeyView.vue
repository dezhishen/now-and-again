...
const SCOPE_OPTIONS = [
  { group: '家庭', items: [
    { value: 'family:read', label: '查看家庭信息' },
    { value: 'family:write', label: '修改家庭资源' },
    { value: 'family:admin', label: '管理成员与角色' },
  ]},
  { group: '户型图', items: [
    { value: 'floorplan:read', label: '查看户型图' },
    { value: 'floorplan:write', label: '上传/编辑户型图' },
  ]},
  { group: '任务', items: [
    { value: 'task:read', label: '查看任务与待办' },
    { value: 'task:write', label: '创建/编辑任务' },
  ]},
  { group: '日历', items: [
    { value: 'ics:read', label: '读取日历订阅' },
  ]},
  { group: '个人信息', items: [
    { value: 'user:read', label: '查看个人信息' },
  ]},
  { group: '管理', items: [
    { value: 'admin:read', label: '查看管理面板' },
    { value: 'admin:write', label: '修改系统设置' },
  ]},
]
...
<template>
  <div class="max-w-2xl mx-auto py-4 md:py-8 px-4">
    ...
    <div v-if="createdKey" class="card mb-4 border border-yellow-300 dark:border-yellow-700 bg-yellow-50 dark:bg-yellow-900/20">
      ...
      <button class="btn-primary text-xs" @click="copyKey">复制并关闭</button>
    </div>

    <button class="btn-primary text-sm mb-4" @click="showCreate = !showCreate">{{ showCreate ? '取消' : '+ 创建 API Key' }}</button>

    <!-- Create form -->
    <div v-if="showCreate" class="card mb-4 space-y-3">
      <input v-model="newName" class="input" placeholder="Key 名称，如：家庭自动化脚本" />
      <div>
        <p class="text-xs text-gray-400 mb-2">权限范围（不选则为全部权限）：</p>
        <!-- Quick shortcuts -->
        <div class="flex flex-wrap gap-1.5 mb-3">
          ...
          >{{ grp === 'read' ? '📖 只读' : grp === 'write' ? '✏️ 读写' : '🔧 管理员' }}</button>
        </div>
        <!-- Grouped scopes -->
        <div v-for="g in SCOPE_OPTIONS" :key="g.group" class="mb-2">
          <p class="text-xs text-gray-500 mb-1">{{ g.group }}</p>
    ...
    <div v-for="k in keys" :key="k.id" class="card mb-2">
      <div class="flex items-center justify-between">
        ...
        <button class="text-xs text-danger hover:underline" @click="revoke(k.id)">撤销</button>
      </div>
      <div class="flex gap-3 mt-1.5 text-xs text-gray-400">
        <span v-if="k.scopes?.length">
          <span v-for="(s, i) in k.scopes" :key="s">
            <span v-if="i > 0">, </span>{{ SCOPE_LABELS[s] || s }}
          </span>
        </span>
        <span v-else class="text-primary font-medium">全部权限</span>
        <span v-if="k.expires_at">过期: {{ new Date(k.expires_at).toLocaleString() }}</span>
        <span v-else>永不过期</span>
        <span v-if="k.last_used_at">最近: {{ new Date(k.last_used_at).toLocaleString() }}</span>
      </div>
    </div>
