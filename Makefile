...
db-reset: ## 删除整个 SQLite 数据库文件
	@rm -f data/*.db data/*.db-journal data/*.db-wal data/*.db-shm
	@echo "→ database removed"

db-clear-logs: ## 清空操作日志和待办（保留用户/家庭/任务模板）
	@python3 -c "\
import sqlite3, glob, sys; \
db = glob.glob('data/*.db'); \
if not db: print('no database found'); sys.exit(1); \
conn = sqlite3.connect(db[0]); \
for t in ['task_logs', 'todos']: conn.execute(f'DELETE FROM {t}'); print(f'  cleared: {t}'); \
